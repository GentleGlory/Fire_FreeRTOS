...
#define KEY_DOWN_EVENT    ( 1 << 0)
#define KEY_UP_EVENT    ( 1 << 0)
	  ...
	  while(1)
	  {
		    ulReturn = osThreadFlagsWait( MAX_THREAD_FLAGS, osFlagsWaitAny, osWaitForever );
			  ...
			  if( (ulReturn & osFlagsError) ) 
				{
					  printf( "LED_Task wait for flags failed. error:%u \r\n ", ulReturn );
				}
				else
        {
				    if( (ulReturn & KEY_DOWN_EVENT) )
						{
						    printf( "Got key down event.\r\n ");
                HAL_GPIO_TogglePin( GPIOD, LD3_Pin);
						}
						
						if( (ulReturn & KEY_UP_EVENT) )
						{
    ...
    while(1)
    {
        if( HAL_GPIO_ReadPin(B1_GPIO_Port, B1_Pin) == GPIO_PIN_SET )
        {
            if( pressed == pdFALSE )
            {
                ...
                pressed = pdTRUE;
                ulReturn = osThreadFlagsSet(LED_Task_Handle, KEY_DOWN_EVENT );

                if( ( ulReturn & osFlagsError ) )
                {
                    printf(" Send key down event to LED_Task. error:%u \r\n", ulReturn);
                }
                else
                {
                    printf(" Send key down event to LED_Task successfully. \r\n");
                }
            }                        
        }
        else
        {
            if( pressed == pdTRUE )
            {
                ...
                ulReturn = osThreadFlagsSet(LED_Task_Handle, KEY_UP_EVENT );

                if( ( ulReturn & osFlagsError ) )
                {
                    printf(" Send key up event to LED_Task. error:%u \r\n", ulReturn);
                }
                else
                {
                    printf(" Send key up event to LED_Task successfully. \r\n");
                }
            }
        }
