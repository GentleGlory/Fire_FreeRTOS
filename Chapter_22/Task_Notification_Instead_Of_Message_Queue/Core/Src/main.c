...
static void Receive1_Task(void* parameter)
{
    ...
    while(1)
    {
        ...
        {
#if USE_CHAR
            printf("Receive1_Task got notification:%s \r\n", (char*) ulReturn);
#else
            printf("Receive1_Task got notification:%u \r\n", ulReturn);
#endif             
        }
...
static void Receive2_Task(void* parameter)
{
    ...
    while(1)
    {
        ...
        {
#if USE_CHAR
            printf("Receive2_Task got notification:%s \r\n", (char*) ulReturn);
#else
            printf("Receive2_Task got notification:%u \r\n", ulReturn);
#endif             
        }
    ...
    while(1)
    {
        if( HAL_GPIO_ReadPin(B1_GPIO_Port, B1_Pin) == GPIO_PIN_SET )
        {
            if( pressed == pdFALSE )
            {
                ...
                pressed = pdTRUE;
#if  USE_CHAR              
                ulReturn = osThreadFlagsSet(Receive1_Task_Handle, (uint32_t) test_str1 );
#else
                ulReturn = osThreadFlagsSet(Receive1_Task_Handle, send_data1 );
#endif                
...
#if  USE_CHAR              
                ulReturn = osThreadFlagsSet(Receive2_Task_Handle, (uint32_t) test_str2 );
#else
                ulReturn = osThreadFlagsSet(Receive2_Task_Handle, send_data2 );
#endif                
